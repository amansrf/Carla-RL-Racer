...
from perfect_agent_fk import RLe2ePPOEvalAgent
...
def compute_score(carla_runner: CarlaRunner) -> Tuple[float, int, int]:
    ...
    """
    sim_time_elapsed: float = carla_runner.end_simulation_time - carla_runner.start_simulation_time
    real_time_elapsed: float = carla_runner.end_real_time - carla_runner.start_real_time
    ...
    return real_time_elapsed, sim_time_elapsed, num_collision, laps_completed
...
def main():
    ...
    num_laps = 1
    table = PrettyTable()
    table.field_names = ["real_time_elapsed(sec)", "sim_time_elapsed (sec)", "num_collisions", "laps completed"]
